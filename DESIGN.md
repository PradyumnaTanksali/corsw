...
### Primary
- **Vermillion** (`paper-vermillion`, Paper's `--accent`) and **Electric Blue** (`schematic-blue`, Schematic's `--accent`): the accent role. It carries the masthead line, the ordinals on project cards and in the Manifesto, one emphasis word per section headline, the Index reference codes, link hovers and the drawn underline, diagram connectors and the core diagram box, the `/demo` call to action, the colophon square, the selection highlight and focus outlines on the theme toggle and the demo button.
...
- **Ink** (`paper-ink` / `schematic-ink`, `--ink`): headlines, principle lines, table values, the active toggle state, and Provenance's lead paragraph.
- **Muted Ink** (`paper-ink-muted` / `schematic-ink-muted`, `--ink-muted`): body paragraphs, the nameplate row, section-folio labels and their ordinals, secondary link text, and the second line of the `/demo` headline.
- **Faint Ink** (`paper-ink-faint` / `schematic-ink-faint`, `--ink-faint`): table row labels, reference metadata, diagram sub-labels and annotations, the `·` and `/` separators, inactive toggle buttons, the copyright line.
- **Rule** (`paper-rule` / `schematic-rule`, `--ink-rule`): every hairline, including section folios, card separators, table and list rows, the diagram panel border, and the masthead dateline.
...
**The Accent Marks, Never Fills Rule.** The accent is a 1px line, a word, a numeral, a code or a border. The only accent areas are the 8px colophon square and the `/demo` button's hover fill.
...
- **Body Large** (400, 18px, 1.45): Manifesto principle lines in Ink, with ordinals at 24px, or 30px from `md`.
...
- **Mono Caps** (400, 11px, 0.22em, uppercase; 0.16em below `sm` in the masthead): the nameplate row, theme toggle, copyright line and `/demo` footer.
- **Mono Data** (400, 13px, 1.7, tabular): Index and Ledger rows, the colophon, the masthead dateline and `/demo` links. It drops to 12.5px for project data tables and the project name row, and to 12px for card links.
- **Mono Tag** (400, 10.5–11px, 0.12em, uppercase): the diagram panel header and the status badge. The same 0.12em caps run at 12.5px for project data-table row labels and the project name, and the Index row labels use 13px at 0.18em.
...
The masthead is a nameplate row, a display headline 64–96px below it, and a dateline row 64–96px further down behind a hairline. A 1px accent line hangs at the right edge (16px in, 64px from `md`) and grows to 60vh.
...
**The Hairline Rhythm Rule.** Separation is a 1px `--ink-rule` top border: folios, card boundaries, table rows, list rows, the dateline. Sections are never boxed, and the Manifesto and data rows never alternate background colors.
...
- **Primary (`/demo` only):** Mono Data at 13px in the accent, 12px × 20px padding, label followed by `→`.
...
- **Ledger rows:** the whole row is the link. On hover, the hostname turns accent with a 4px-offset accent underline and the trailing arrow turns accent.
...
### Data Rows
- **Data Table (project cards):** mono 12.5px, tabular. Each row has a 1px Rule top border, and the last row also has a bottom border. Row labels are uppercase 0.12em Faint Ink in a 112px column. Values are Ink. The reference code column (56px, right-aligned, Faint Ink) shows from `sm` only.
- **Index:** a `<dl>` in Mono Data on the 8-column side. Rows have 12px vertical padding with a Rule top border (none on the first row) and four columns: the label (uppercase 0.18em, Faint Ink), the value in Ink, a note in Muted Ink, and the reference code in the accent, right-aligned and shown only from `md`.
- **Ledger:** the same rhythm with 16px row padding, hostname / label / arrow columns, and the full row as the link.
...
- **Colophon:** a centered footer behind a Rule top border, 48px of top padding and a 24px gap between items: Mono Data at 13px in Muted Ink, a Mono Caps copyright line in Faint Ink, and an 8px accent square as the full stop.
